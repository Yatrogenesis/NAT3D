...
fn test_render_farm_heartbeat_determinism_n30() {
    for i in 0..30 {
        ...
        // No timeouts yet
        assert_eq!(
            monitor.check_timeouts().len(),
            0,
            "Run {}: No timeouts expected",
            i
        );
        ...
        assert_eq!(
            timed_out_after_revive.len(),
            9,
            "Run {}: Only 9 workers should timeout",
            i
        );
        assert!(
            !timed_out_after_revive.contains(&workers[0]),
            "Run {}: Revived worker should not timeout",
            i
        );
    }
