...
use crate::protocol::{SyncMessage, TileSpec, RenderResult};
...
impl RenderFarmWorker {
    ...
    pub async fn render_tile(&self, tile: &TileSpec, frame: u32) -> anyhow::Result<Vec<u8>> {
        tracing::debug!("REAL Rendering tile {}x{} for frame {}", tile.width, tile.height, frame);
...
mod tests {
    ...
    async fn test_worker_frame_variation() {
        ...
        let spec = TileSpec { x: 0, y: 0, width: 64, height: 64 };
        
        let t0 = worker.render_tile(&spec, 0).await.unwrap();
        let t1 = worker.render_tile(&spec, 30).await.unwrap();
        
        assert_ne!(t0, t1, "Tiles with different frames must produce different output");
    }
}
