...
// ── NeRF volumetric accumulation ────────────────────────────────────────────

#[test]
fn e2e_nerf_accumulate_zero_density_is_black() {
    use nalgebra::Vector3;
    ...
    let samples: Vec<VolumeSample> = (0..16)
        .map(|_| VolumeSample {
            ...
            density: 0.0, // no density → no contribution
        })
        .collect();

    let rgb = accumulate_radiance(&samples, 0.1);
    assert!(
        rgb.norm() < 1e-9,
        "zero-density ray should produce black, got {:?}",
        rgb
    );
}

#[test]
fn e2e_nerf_accumulate_opaque_surface_saturates() {
    use nalgebra::Vector3;
    use nat3d_render::raytracing::nerf::{accumulate_radiance, VolumeSample};

    // First sample has near-infinite density → should capture almost all light
    let mut samples = vec![VolumeSample {
        color: Vector3::new(0.8, 0.5, 0.2),
        density: 1000.0,
    }];
    for _ in 1..16 {
    ...
    let rgb = accumulate_radiance(&samples, 0.1);
    // Result should be close to the first sample's color
    assert!((rgb.x - 0.8).abs() < 0.01, "r={}", rgb.x);
    assert!((rgb.y - 0.5).abs() < 0.01, "g={}", rgb.y);
    assert!((rgb.z - 0.2).abs() < 0.01, "b={}", rgb.z);
}

// ── NRC MLP forward pass ────────────────────────────────────────────────────

#[test]
fn e2e_nrc_mlp_output_in_unit_range() {
    use nalgebra::DVector;
    use nat3d_render::raytracing::nrc::RadianceMLP;

    let mlp = RadianceMLP::new_random(5, 64, 3);
    let input = DVector::from_vec(vec![0.1, 0.2, 0.3, 0.5, 1.0]);
    let rgb = mlp.predict(input);

    // Sigmoid output must be in [0, 1]
    assert!(rgb.x >= 0.0 && rgb.x <= 1.0, "r out of range: {}", rgb.x);
    assert!(rgb.y >= 0.0 && rgb.y <= 1.0, "g out of range: {}", rgb.y);
    assert!(rgb.z >= 0.0 && rgb.z <= 1.0, "b out of range: {}", rgb.z);
}

#[test]
fn e2e_nrc_mlp_deterministic_for_same_input() {
    use nalgebra::DVector;
    use nat3d_render::raytracing::nrc::RadianceMLP;

    let mlp = RadianceMLP::new_random(5, 32, 3);
    let input = DVector::from_vec(vec![0.5, 0.5, 0.5, 1.0, 0.0]);
    let r1 = mlp.predict(input.clone());
    let r2 = mlp.predict(input);

    assert!((r1 - r2).norm() < 1e-12, "MLP must be deterministic");
}
